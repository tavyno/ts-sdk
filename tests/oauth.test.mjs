...
const metadata = { issuer: 'https://identity.example/', authorization_endpoint: 'https://identity.example/authorize',
  token_endpoint: 'https://identity.example/token', jwks_uri: 'https://identity.example/jwks', end_session_endpoint: 'https://identity.example/logout',
  response_types_supported: ['code'], subject_types_supported: ['public'], id_token_signing_alg_values_supported: ['RS256'] };
test('creates provider-neutral PKCE transactions without browser navigation or client secrets', async () => {
  const client = createOAuthClient({ issuer: metadata.issuer, clientId: 'public-client', audience: 'api', fetcher: async () => Response.json(metadata) });
...
test('exchanges public-client codes, validates ID-token signatures and refreshes tokens without runtime dependencies', async () => {
  const keys = await globalThis.crypto.subtle.generateKey({ name: 'RSASSA-PKCS1-v1_5', modulusLength: 2048, publicExponent: new Uint8Array([1, 0, 1]), hash: 'SHA-256' }, true, ['sign', 'verify']);
  ...
  async function idToken(nonce) {
    ...
    const signature = await globalThis.crypto.subtle.sign('RSASSA-PKCS1-v1_5', keys.privateKey, Buffer.from(payload));
    return `${payload}.${Buffer.from(signature).toString('base64url')}`;
  }
  const client = createOAuthClient({ issuer: metadata.issuer, clientId: 'client', fetcher: async (input, init) => {
    const url = String(input);
    if (url.includes('openid-configuration')) { return Response.json(metadata); }
    if (url.endsWith('/jwks')) { return Response.json({ keys: [{ ...publicKey, kid: 'key', alg: 'RS256', use: 'sig' }] }); }
    tokenRequests++;
    ...
    if (body.get('grant_type') === 'refresh_token') {
      ...
      return Response.json({ access_token: 'renewed', refresh_token: 'rotated', token_type: 'Bearer', expires_in: 300 });
    }
    assert.equal(body.get('code_verifier'), transaction.codeVerifier);
    return Response.json({ access_token: 'jwt', refresh_token: 'refresh', token_type: 'Bearer', expires_in: 300, id_token: await idToken(transaction.nonce) });
  } });
  ...
  const tokens = await client.completeLogin(`https://web.example/callback?code=code&state=${transaction.state}`, transaction);
  ...
  await assert.rejects(client.completeLogin(`https://web.example/callback?code=code&state=${transaction.state}`, { ...transaction, expiresAt: 0 }));
  const logout = new globalThis.URL(await client.createLogoutUrl({ returnTo: 'https://web.example/', idToken: tokens.idToken }));
